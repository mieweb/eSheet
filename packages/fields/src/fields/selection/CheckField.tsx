...
export const CheckField = React.memo(function CheckField({
  ...
  if (isPreview) {
    return (
      <div className="check-field-preview es:grid es:grid-cols-1 es:gap-2 es:sm:grid-cols-2 es:pb-4">
        ...
        <div className="es:space-y-2">
          {options.map((option) => (
            <label
              key={option.id}
              className="es:flex es:items-center es:gap-3 es:px-3 es:py-2 es:cursor-pointer es:rounded-lg es:hover:bg-esprimary/10 es:transition-colors"
            >
              <CustomCheckbox
                ...
                size="lg"
              />
              <span className="es:text-estext es:break-words es:overflow-hidden">
                {option.value}
              </span>
            </label>
          ))}
  ...
  return (
    <div className="check-field-edit es:space-y-3">
      <div>
        <label
      ...
      <div>
        ...
        <div className="es:space-y-2">
          {options.map((option) => (
            ...
            >
              <span className="es:shrink-0 es:w-4 es:h-4 es:rounded es:border es:border-esborder es:bg-essurface" />
              <input
