...
export const BUILDER_TOOL_DEFINITIONS: readonly ToolDefinition[] = [
  ...
  },
  {
    ...
    function: {
      name: 'create_field',
      description:
        'Add one field. fieldType: text|longtext|richtext|radio|check|boolean|dropdown|multiselectdropdown|rating|ranking|slider|section|html|display|image|signature|diagram. properties: {inputType,unit,min,max,step}. Pass parentId to place the field directly inside a section. IMPORTANT: section fields cannot be placed inside another section — sections are always at the root level. For "html" fields: set properties.htmlContent to a raw HTML string (e.g. "<p>Your message</p>") — do NOT use "question". For "display" fields: set properties.content to a markdown/formula string that may embed field values using {fieldId} syntax (e.g. "Your score is **{score-field}**") — do NOT use "question". For "section" fields: use properties.title instead of question. For "richtext" fields: optionally set properties.defaultContent to a Markdown string that pre-fills the editor (e.g. "## Heading\n\nParagraph text here.").',
      parameters: {
        type: 'object',
        properties: {
